// Alumno: Jaime Orlando López Ramos, A01374696
// Descripción: Desarrollo de una app sencilla en WebGL


// Usamos gl Matrix para nuestra generación de Matrices
mat4 = glMatrix.mat4;
...
// Definimos el Código Fuente del Vertex Shader
...
// Definimos el Código Fuente del Fragment Shader
...
function main()
{
    ...
    let vertsPacman = [2, -.5, 0];

    // Creamos Nuestras Figuras
    let square = createSquare(gl, vertsSq);
    let triangle = createTriangle(gl, vertsTr);
    let rhombus = createSquare(gl, vertsRhomb);
    let pacman = createSphere(gl, 0.5, vertsPacman, 2, -0.5);
...
// Función que crea un Cuadrado dado un arreglo con las coordenadas de sus vértices
function createSquare(gl, verts)
{
    let vertexBuffer;
...
// Función que crea un Triángulo dado un arreglo con las coordenadas de sus vértices
function createTriangle(gl, verts)
{
    let vertexBuffer;
    vertexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
...
// Función que crea un Círculo, dado un arreglo de vértices incial, su radio y el desplazamiento tanto en el eje x como en el y
function createSphere(gl, radius, verts, despX, despY)
{
    let vertexBuffer;
    vertexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
    ...
    // Definimos Ángulos Iniciales y Finales del Círculo
    let startCircle = 45;
    let endCircle = 325;
    ...
    {
        //x
        verts.push(despX + (Math.cos(degreesToRadians(i)) * radius));
        //y
        verts.push(despY + Math.sin(degreesToRadians(i)) * radius);
